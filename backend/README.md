...
- **接口**: `GET /api/records/statistics/jump-fiber`
- **参数**: 
  - `datacenter_name`: 机房名称（可选）
  - `status`: 线路状态（可选，0-全部，1-在用，2-已拆除）

- **请求示例**:

```bash
curl "http://localhost:3001/api/records/statistics/jump-fiber"
```
...
```json
{
  ...
  "data": {
    ...
    "datacenters": [
      {
        "datacenter_name": "临港机房",
        "total_count": 45,
        "in_use_count": 38,
        "removed_count": 7,
        "in_use_percentage": 84.44,
        "removed_percentage": 15.56
      },
      {
        "datacenter_name": "金桥机房",
        "total_count": 32,
        "in_use_count": 28,
        "removed_count": 4,
        "in_use_percentage": 87.50,
        "removed_percentage": 12.50
      }
    ]
  }
}
...
- **接口**: `GET /api/records/statistics/jump-fiber/export`
- **参数**: 无（导出所有机房的统计数据）
- **返回**: 
  - Excel文件下载
  - Content-Type: `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`
  - Content-Disposition: `attachment; filename*=UTF-8''跳纤统计报表_YYYY-MM-DD.xlsx`

- **请求示例**:

```bash
curl -O -J "http://localhost:3001/api/records/statistics/jump-fiber/export"
```

- **Excel文件内容**:

1. **总体统计部分**
   - 统计项目：总体统计、在用线路总数、已拆除线路总数
   - 数值：对应的统计数量
   - 说明：统计项的详细说明

2. **机房详细统计部分**
   - 表头：机房名称、跳纤总数、在用线路、在用比例、已拆除线路、已拆除比例
   - 数据：每个机房的统计信息
   - 比例格式：百分比（保留两位小数）

3. **导出信息部分**
   - 导出时间：导出操作的时间戳（格式：YYYY-MM-DD HH:mm:ss）
   - 数据来源：综合布线记录管理系统

- **示例文件名**:
  ```
  跳纤统计报表_2026-01-12.xlsx
  ```
